...
const Urls = {
  ...
  getTransactionsById:({pathParams, bodyParams}) => {
      return {
          url:`https://api.mch.weixin.qq.com/v3/pay/transactions/id/${pathParams.transaction_id}?mchid=${bodyParams.mchid}`,
          method:`GET`,
          pathname:`/v3/pay/transactions/id/${pathParams.transaction_id}?mchid=${bodyParams.mchid}`,
      }
  },
  getTransactionsByOutTradeNo:({pathParams, bodyParams}) => {
      return {
          url:`https://api.mch.weixin.qq.com/v3/pay/transactions/out-trade-no/${pathParams.out_trade_no}?mchid=${bodyParams.mchid}`,
          method:`GET`,
          pathname:`/v3/pay/transactions/out-trade-no/${pathParams.out_trade_no}?mchid=${bodyParams.mchid}`,
      }
  },
  close:({pathParams}) => {
      return {
...
class Payment extends Service {
  async run ({pathParams, queryParams, bodyParams, type}) {
    let { mchid, private_key } = this.app.config.wechatPay;
    let {url, method, pathname, serial_no} = Urls[type]({ pathParams, queryParams, bodyParams });
  ...
  //jsapi统一下单
  async jsapi(params){
    let { appid, mchid, notify_url } = this.app.config.wechatPay;
    let bodyParams = {
      ...params,
      appid,
      mchid,
  ...
  //关闭订单
  async close(params){
    let { mchid } = this.app.config.wechatPay;
  ...
  //退款
  async refund(params){
    let { notify_url } = this.app.config.wechatPay;
    let bodyParams = {
        ...params,
